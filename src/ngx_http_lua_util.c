...
char*
ngx_http_lua_arg_compile(ngx_conf_t *cf, ngx_http_lua_arg_tmpl_t *res, ngx_str_t *raw_val)
{
    ngx_uint_t  n;
    ...
    res->raw_value = *raw_val;
    res->lengths = NULL;
    res->values = NULL;

    n = ngx_http_script_variables_count(&res->raw_value);

    if (n > 0) {
        ngx_memzero(&sc, sizeof(sc));

        sc.cf = cf;
        sc.source = &res->raw_value;
        sc.lengths = &res->lengths;
        sc.values = &res->values;
        sc.variables = n;
...
ngx_int_t
ngx_http_lua_arg_eval(ngx_http_request_t *r, ngx_str_t *res, ngx_http_lua_arg_tmpl_t *arg)
{
    ...
    if (arg->lengths == NULL) {
        *res = arg->raw_value;
        ...
        if (ngx_http_script_run(r, res,
                    arg->lengths->elts, 0, arg->values->elts) == NULL) {
            return NGX_ERROR;
        }
    }
