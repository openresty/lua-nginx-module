...
ngx_http_lua_init_main_conf(ngx_conf_t *cf, void *conf)
{
    ...
    if (ngx_http_lua_init_builtin_headers_in(cf, lmcf) != NGX_OK) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "init header in error");

        return NGX_CONF_ERROR;
    }

    dd("init built in headers in hash size: %ld",
